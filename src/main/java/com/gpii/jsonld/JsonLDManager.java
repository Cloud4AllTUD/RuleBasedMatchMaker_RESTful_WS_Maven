...
public class JsonLDManager {
  ...
  private JsonLDManager() {
    ...
    try {
      ...
      /**
       * TODO find a new location, not in folder test data; split ontology alignment rules from
       * matching rules
       */
      ...
      solutionsOntology =
          System.getProperty("user.dir") + WEBINF_PATH + prop.getProperty("solutionsOntology");

      // debug
      initialJsonInputFilepath =
          System.getProperty("user.dir") + WEBINF_PATH
              + prop.getProperty("initialJsonInputFilepath");
      transformedJsonLDInputFilepath =
          System.getProperty("user.dir") + WEBINF_PATH
              + prop.getProperty("transformedJsonLDInputFilepath");
      initialOntModelFilepath =
          System.getProperty("user.dir") + WEBINF_PATH
              + prop.getProperty("initialOntModelFilepath");
      inferredOntModelFilepath =
          System.getProperty("user.dir") + WEBINF_PATH
              + prop.getProperty("inferredOntModelFilepath");
      rbmmJsonOutputFilepath =
      ...
      INTEGRATION_TESTS_INCLUDE_ONTOLOGY_TRANSFORMATION_INTO_JSONLD =
          Boolean.parseBoolean(prop
              .getProperty("INTEGRATION_TESTS_INCLUDE_ONTOLOGY_TRANSFORMATION_INTO_JSONLD"));
  ...
  public String runJSONLDTests(String tmpInputJsonStr) throws IOException, JSONException {
    ...
    Utils.getInstance().writeFile(initialJsonInputFilepath,
        Utils.getInstance().jsonPrettyPrint(tmpInputJsonStr));
    ...
    Utils.getInstance().writeFile(transformedJsonLDInputFilepath,
        Utils.getInstance().jsonPrettyPrint(transIn));
    ...
    Utils.getInstance().writeFile(rbmmJsonOutputFilepath,
        Utils.getInstance().jsonPrettyPrint(resJsonStr));
